...
const regex = /      <!-- Trust Bar \(Top Footer\) -->[\s\S]*?<\/div>\n      <\/div>\n\n      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-8 lg:gap-10">/;

const newTrustBar = `      <!-- Trust Bar (Top Footer) -->
      <div class="border-b border-gray-200 mb-12">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 divide-y md:divide-y-0 lg:divide-x divide-gray-200 py-8">
...
if (content.match(regex)) {
    content = content.replace(regex, newTrustBar);
    fs.writeFileSync('frontend/src/main.js', content, 'utf8');
    console.log("Applied final tweaks to Trust Bar.");
} else {
    console.log("Could not find the Trust Bar.");
}
