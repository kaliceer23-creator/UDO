...
const oldDock = `  <!-- Fixed Bottom Dock (UDO Custom) -->
...
const newDock = oldDock.replace(/stroke-width="1\.8"/g, 'stroke-width="1.4"').replace(/stroke-width="1\.5"/g, 'stroke-width="1.2"');

if (content.includes(oldDock)) {
    content = content.replace(oldDock, newDock);
    fs.writeFileSync('frontend/src/main.js', content, 'utf8');
    console.log("Dock icons stroke-width reduced to 1.4");
} else {
    console.log("Could not find the dock section to replace.");
}
