...
const startMarker = '      <!-- เมนูตรงกลาง (Center Aligned) -->';
const endMarker = '      <!-- บล็อกสมดุลฝั่งขวา';
    ...
    const newLinks = `      <!-- เมนูตรงกลาง (Center Aligned) -->
      <div class="hidden lg:flex flex-1 justify-center h-full items-center gap-6 xl:gap-8 whitespace-nowrap">
        ...
        <div class="relative h-full flex items-center group/dropdown" tabindex="0">
          <!-- Text and Arrow -->
          <div class="flex items-center gap-1 text-white group-hover/dropdown:text-brand-red font-medium text-[13.5px] md:text-[14px] transition-colors h-full cursor-pointer px-1">
            กลุ่มลวดเชื่อม
            <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5 text-white group-hover/dropdown:text-brand-red transition-all duration-300 group-hover/dropdown:-rotate-180" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" /></svg>
          </div>
          
          <!-- Dropdown Box (8 Categories) -->
          <div class="hidden group-hover/dropdown:block group-focus-within/dropdown:block absolute top-[100%] left-1/2 -translate-x-1/2 w-[220px] bg-white rounded-b-lg shadow-[0_10px_40px_rgba(0,0,0,0.1)] border-t-[3px] border-brand-red py-2 z-[60]">
          ...
          <!-- Thick Red Underline -->
          <span class="absolute bottom-0 left-0 w-full h-[3px] bg-brand-red opacity-0 group-hover/dropdown:opacity-100 transition-opacity z-[61]"></span>
        </div>
...
`;
    ...
    console.log("Navbar updated based on new rules!");
} else {
